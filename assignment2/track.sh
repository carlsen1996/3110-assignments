...
track () {
	...
	if [ $1 == "start" ]; then
		if [[ $(tail -1 ./LOGFILE.txt) == *"LABEL"* ]]; then
			echo "You can only have one timer running."
			echo "Use track stop to stop the current timer."
		else
			label="$2"
			start="$(date +%s)"
			echo "START " $(date) >> LOGFILE.txt
			echo "LABEL This is $2" >> LOGFILE.txt 
		fi
		
	elif [ $1 == "stop" ]; then
		if [[ $(tail -2 ./LOGFILE.txt) == *"END"* ]]; then
			...
			echo "END " $(date) >> LOGFILE.txt
			echo "" >> LOGFILE.txt ##to make a newline between the tasks
		fi
